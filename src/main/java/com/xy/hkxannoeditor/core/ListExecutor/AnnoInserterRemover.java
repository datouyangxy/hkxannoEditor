...
import com.xy.hkxannoeditor.utils.CommonUtil;
...
public class AnnoInserterRemover extends ListFixExecutor {
    ...
    private void inserterProcess(double frame, boolean insertAfter) {
        while (_reader.hasNextLine()) {
            ...
            if (line.contains(_line)) {
                ...
                if (insertAfter) {
                    String newLine = String.format(_template, CommonUtil.addFrame(prevFrame, frame));
                    _lines.add(newLine);
    ...
    private void repetitionRemoverProcess() {
        ...
        while (_reader.hasNextLine()) {
            String line = _reader.nextLine();
            if (line.contains("animmotion") && AMRannoSpliter(line).equals(prevPos)) {
    ...
    public boolean insertAfter() throws IOException {
        inserterProcess(Const.ONE_FRAME, true);
        _reader.close();
        writeAnno();
        return _fixed;
    }
    ...
    public boolean insertBefore() throws IOException {
        inserterProcess(Const.ONE_FRAME, false);
        _reader.close();
        writeAnno();
        return _fixed;
    }
